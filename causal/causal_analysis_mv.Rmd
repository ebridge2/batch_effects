...
pval_hmap_chart <- function(data, title="", method="Raw", text.size=15, preproc=TRUE) {
  ...
  data <- data %>%
    mutate(Dataset.tmp = as.character(Dataset.Trt),
           Dataset.Trt=ifelse(Data == "Site Effect", as.character(Dataset.Ctrl), as.character(Dataset.Trt)),
           Dataset.Ctrl=ifelse(Data == "Site Effect", as.character(Dataset.tmp), as.character(Dataset.Ctrl)),
           Dataset.Trt=factor(Dataset.Trt, 
                              levels=(dset.ord %>% arrange(n, rev(as.character(Dataset))))$Dataset, 
                              ordered=TRUE),
           Dataset.Ctrl=factor(Dataset.Ctrl, 
  ...
  data %>%
    ...
    scale_fill_manual(values=c("Site Effect"="dark green", "Independence"="red"), name="Estimated Effect") +
    ggtitle(title) +
    theme_bw() +
...
pval_hist_chart <- function(data, nbreaks=30, title="", y.scale=.03, text.y=.4, method="Raw",
  ...
  plt <- data %>%
    ...
    ggplot() +
      ...
      scale_fill_manual(values=c("Independence"="red", "Site Effect"="darkgreen"), name="Estimated Effect") +
