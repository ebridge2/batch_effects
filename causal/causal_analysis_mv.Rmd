...
rbind(get_connectomes_dataset(results$Stats, dataset="BNU1"),
  ...
  ggplot(aes(x=reorder(Row, Row.mod2), y=reorder(Column, Col.mod2), fill=Value)) +
    ...
    ggtitle("Raw fMRI Connectomes from CoRR Meta-Dataset") +
...
results$Site <- results$Site %>%
  ...
  mutate(p.value=p.adjust(p.value, method="BH"),
         ...
         Dataset.Ctrl=factor(Dataset.Ctrl, 
                             levels=dset.ord.site$Dataset, 
                             ordered=TRUE)) %>%
  mutate(Data=factor(Data, levels=c("Associational", "Causal Obs.", "Causal Exp."), ordered=TRUE))
...
pval_hmap_chart <- function(data, dset.ord.site, title="", method="Raw", text.size=15, preproc=TRUE) {
  ...
  data <- data %>%
    ...
    mutate(Dataset.Trt.X=c(Dataset.Trt, Dataset.Ctrl)[which.min(c(id.trt, id.ctrl))],
           ...
           Dataset.Trt=ifelse(grepl("Causal", Data), as.character(Dataset.Ctrl), as.character(Dataset.Trt)),
           Dataset.Ctrl=ifelse(grepl("Causal", Data), as.character(Dataset.tmp), as.character(Dataset.Ctrl)),
           Dataset.Trt=factor(Dataset.Trt,
                              levels=dset.ord.site$Dataset,
                              ordered=TRUE),
           Dataset.Ctrl=factor(Dataset.Ctrl,
  ...
  data %>%
    ...
    scale_fill_manual(values=c("Causal Obs."="dark green", "Causal Exp."="purple", "Associational"="red"), 
...
pval_hist_chart <- function(data, nbreaks=30, title="", text.y=.8, method="Raw",
  ...
  plt <- data %>%
    ...
    ggplot() +
      ...
      scale_fill_manual(values=c("Associational"="red", "Causal Obs."="darkgreen", "Causal Exp."="purple"), name="Estimated Effect") +
      scale_color_manual(values=c("Associational"="red", "Causal Obs."="darkgreen", "Causal Exp."="purple")) +
...
compare_site <- function(data, dset.ord.site, method="Raw", nbreaks=30, text.y=.8, y.scale=.03, title.hmap="",
  ...
  arrangeGrob(p.hmap, p.hist, heights=c(.7, .25), nrow=2)
}
```
...
results$Site <- results$Site %>%
  mutate(Method=factor(Method, levels=c("Raw", "ComBat", "Ranked", "Z-Score"), ordered=TRUE))  %>%
...
hist.site <- results$Site %>%
  ...
  geom_histogram(aes(p.value, color=Data, fill=Data,
      ...
      scale_fill_manual(values=c("Associational"="red", "Causal Obs."="darkgreen", "Causal Exp."="purple"), name="Estimated Effect") +
      scale_color_manual(values=c("Associational"="red", "Causal Obs."="darkgreen", "Causal Exp."="purple"), name="Estimated Effect") +
...
cov.combt <- results$Covariate %>% filter(Method == "causal ComBat")
results$Covariate <- rbind(cov.combt, results$Covariate %>% 
                             filter(Dataset %in% cov.combt$Dataset & Method != "causal ComBat"))
results.cov <- results$Covariate %>%
  filter(Method != "Raw") %>%
  left_join(results$Covariate %>% filter(Method == "Raw") %>% 
...
con.plt <- get_site_gr(results$Stats, site) %>%
  ...
  ggplot(aes(x=Row, y=reorder(Column, desc(Column)), fill=Value)) +
  geom_tile() +
...
scatter.cov <- ggplot(results.cov %>% slice(sample(1:n()))) +
  geom_point(aes(x=p.value.raw, y=p.value, color=Effect.Name, shape=Effect.Name), size=2) +
  facet_grid(".~Method") +
  scale_color_manual(values=c("Age"="blue2", "Sex"="firebrick2"), name="Covariate") +
...
get_sig_gr <- function(data, site="BNU1") {
  do.call(rbind, lapply(names(data), function(norm) {
    x <- data[[norm]]
...
con.dat <- get_sig_gr(results$Stats) %>%
  mutate(Row.mod2=as.numeric(Row) %% 2, Col.mod2 = as.numeric(Column) %% 2,
         Method=factor(Method, levels=c("Community", "Raw", "ComBat", "Ranked", "Z-Score"), ordered=TRUE)) %>%
...
plt <- results$Signal %>%
  filter(Community != "Upper.Tri" & Method != "cond. ComBat") %>%
