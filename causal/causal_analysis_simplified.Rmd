...
We look at the distribution of $p$-values for the site effects in "invalid" comparisons.

```{r, fig.height=3, fig.width=9}
ignor.ov.dat <- raw$estimates %>%
  filter(Data == "Unmatched")

acc.ov.dat <- raw$estimates %>%
  filter(Method %in% c("IPW", "Matched")) %>%
  mutate(Method=recode_factor(Method, "IPW"="Trimmed")) %>%
  select(Dataset.Ref, Dataset.Tgt, Site, Signal, pvalue.site, pvalue.signal, Dxfm, Sxfm, Method, Data)

dat.cmp <- ignor.ov.dat %>%
  left_join(acc.ov.dat %>% filter(Method == "Trimmed") %>%
              rename(pvalue.site.trimmed=pvalue.site, pvalue.signal.trimmed=pvalue.signal, 
                     Signal.trimmed=Signal, Site.trimmed=Site) %>%
              select(-c("Method", "Data")),
            by=c("Dataset.Ref", "Dataset.Tgt", "Dxfm", "Sxfm")) %>%
  left_join(acc.ov.dat %>% filter(Method == "Matched") %>%
              rename(pvalue.site.matched=pvalue.site, pvalue.signal.matched=pvalue.signal, 
                     Signal.matched=Signal, Site.matched=Site) %>%
              select(-c("Method", "Data")),
            by=c("Dataset.Ref", "Dataset.Tgt", "Dxfm", "Sxfm"))

dat.cmp %>%
  filter(Overlap == 0 & Method == "PDcor") %>%
  mutate(NotValid="Continent") %>%
  rbind(dat.cmp %>% filter(is.na(Site.trimmed) & Method == "PDcor") %>% mutate(NotValid="Trimmed"),
        dat.cmp %>% filter(is.na(Site.matched) & Method == "PDcor") %>% mutate(NotValid="3:1 Matching")) %>%
  mutate(NotValid=factor(NotValid, levels=c("Continent", "Trimmed", "3:1 Matching"))) %>%
  ggplot(aes(pvalue.site, color=Method, fill=Method)) +
    geom_histogram(aes(y=..count../sum(..count..))) +
    geom_rug(color="black") +
    facet_grid("Method ~ NotValid") +
