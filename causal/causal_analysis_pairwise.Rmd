...
raw.stat <- read.csv('../data/summary/batch_statistics.csv') %>%
  ...
  filter(!(Dataset %in% c("BNU2", "NYU_2", "UPSM_1")))
...
raw.stat$Continent <- continent[as.character(raw.stat$Dataset)]
```
...
causal_ana <- function(ref.stat, 
    ...
    do.call(rbind, lapply(dsets.noti, function(datasetj) {
      tryCatch({
        ...
        conf.unm.mar.res <- data.frame(Data="Unmatched", Method="Marginal", Dataset.Ref=dataseti, 
                                   Dataset.Tgt=datasetj, Site=unm.mar.sum["factor(Treatment)1", "Estimate"],
                                   Signal=unm.mar.sum["(Intercept)", "Estimate"])
        ...
        conf.unm.adj.res <- data.frame(Data="Unmatched", Method="Adjusted", Dataset.Ref=dataseti, 
                                   Dataset.Tgt=datasetj, Site=unm.adj.sum["factor(Treatment)1", "Estimate"],
                                   Signal=unm.mar.sum["(Intercept)", "Estimate"])
        ...
        conf.mat.mar.res <- data.frame(Data="Matched", Method="Marginal", Dataset.Ref=dataseti,
                                  Dataset.Tgt=datasetj, Site=mat.mar.sum["factor(Treatment)1", "Estimate"],
                                  Signal=mat.mar.sum["(Intercept)", "Estimate"])
        

        ipw.dat <- compute_propensities(dsij.stat %>% mutate(Treatment = factor(Treatment),
                                                             Sex=factor(Sex)), form=form.ipw.prop) %>%
          mutate(Treatment=factor(Treatment), Sex=factor(Sex), Age=Age-mean(Age))
        ...
        conf.ipw.res <- data.frame(Data="Unmatched", Method="IPW", Dataset.Ref=dataseti,
                                   Dataset.Tgt=datasetj, Site=ipw.adj.sum["Treatment1", "Estimate"],
                                   Signal=ipw.adj.sum["(Intercept)", "Estimate"])
        ...
        conf.mat.adj.res <- data.frame(Data="Matched", Method="Adjusted", Dataset.Ref=dataseti,
                                  Dataset.Tgt=datasetj, Site=mat.adj.sum["factor(Treatment)1", "Estimate"],
                                  Signal=mat.adj.sum["(Intercept)", "Estimate"])
        ...
        conf.mat.res <- data.frame(Data="Matched", Method="Matched", Dataset.Ref=dataseti,
                                   Dataset.Tgt=datasetj, Site=mat.est,
                                   Signal=mat.mat.mod$coefficients["(Intercept)"])
  ...
  if (is.null(raw.dat)) {
    ...
    rezz <- test.res %>%
      ...
      left_join(raw.dat %>% rename(Site.Raw=Site) %>% select(Approach, Dataset.Ref, Dataset.Tgt, Site.Raw),
                by=c("Dataset.Ref", "Dataset.Tgt", "Approach")) %>%
      mutate(Abs.Diff=abs(Site.Xfm) - abs(Site.Raw))
...
raw <- causal_ana(raw.stat, raw.gr, sxfm="Raw", dxfm="Raw", pos.scale=.6)
...
```{r, fig.height=4, fig.width=10}
raw <- causal_ana(raw.stat, raw.gr, sxfm="Ranked", dxfm="Raw", pos.scale=.6)
...
raw <- causal_ana(raw.stat, raw.gr, sxfm="Raw", dxfm="Raw", measure="Difference",
                  pos.scale=.6)
zsc <- causal_ana(raw.stat, raw.gr, raw.dat=raw$estimates, sxfm="Raw", dxfm="Z-Score",
                  pos.scale = .6, measure="Difference")
ranked <- causal_ana(raw.stat, raw.gr, raw.dat=raw$estimates, sxfm="Raw", dxfm="Ranked",
                     pos.scale = .6, measure="Difference")
combt <- causal_ana(raw.stat, raw.gr, raw.dat=raw$estimates, sxfm="Raw", dxfm="ComBat", 
                    pos.scale=.6, measure="Difference")
...
raw <- causal_ana(raw.stat, raw.gr, sxfm="Raw", dxfm="Raw", measure="mean",
                  pos.scale=.6, community="Homophilic")
zsc <- causal_ana(raw.stat, raw.gr, raw.dat=raw$estimates, sxfm="Raw", dxfm="Z-Score",
                  pos.scale = .6, measure="mean", community="Homophilic")
ranked <- causal_ana(raw.stat, raw.gr, raw.dat=raw$estimates, sxfm="Raw", dxfm="Ranked",
                     pos.scale = .6, measure="mean", community="Homophilic")
combt <- causal_ana(raw.stat, raw.gr, raw.dat=raw$estimates, sxfm="Raw", dxfm="ComBat", 
                    pos.scale=.6, measure="mean", community="Homophilic")
...
raw <- causal_ana(raw.stat, raw.gr, sxfm="Raw", dxfm="Raw", measure="Difference",
                  pos.scale=.6, community="Homophilic")
zsc <- causal_ana(raw.stat, raw.gr, raw.dat=raw$estimates, sxfm="Raw", dxfm="Z-Score",
                  pos.scale = .6, measure="Difference", community="Homophilic")
ranked <- causal_ana(raw.stat, raw.gr, raw.dat=raw$estimates, sxfm="Raw", dxfm="Ranked",
                     pos.scale = .6, measure="Difference", community="Homophilic")
combt <- causal_ana(raw.stat, raw.gr, raw.dat=raw$estimates, sxfm="Raw", dxfm="ComBat", 
                    pos.scale=.6, measure="Difference", community="Homophilic")
