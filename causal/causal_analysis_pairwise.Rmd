  ...
  test.res <- do.call(rbind, mclapply(datasets, function(dataseti) {
    ...
    do.call(rbind, lapply(dsets.noti, function(datasetj) {
      tryCatch({
        ...
        marginal.mod <- lm(value ~ factor(Treatment) + factor(Sex) + Age, data=dsij.stat)
        ...
        conf.mar.res <- data.frame(Data="Unmatched", Method="Marginal", Dataset.Ref=dataseti, 
                                   Dataset.Tgt=datasetj, Estimate=mar.sum["factor(Treatment)1", "Estimate"])
        ...
        conf.um.res <- data.frame(Data="Matched", Method="Marginal", Dataset.Ref=dataseti,
                                  Dataset.Tgt=datasetj, Estimate=um.sum["factor(Treatment)1", "Estimate"])
        ...
        mat.est <- coefeq %*% matrix(matched.mod$coefficients, ncol=1)
...
```{r, fig.height=13, fig.width=8, warning=FALSE, message=FALSE}
plot(causal_ana(raw.stat, raw.gr, raw.dat=res$raw, sxfm="Raw", dxfm="Z-Score")$grob)
```
